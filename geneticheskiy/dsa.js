...
function redrawVertexesredrawVertexes(){
...
function startPopulationstartPopulation(firstGeneration){
    let res = [];
    ...
    for (let i = 0; i < vertexes.length * vertexes.length; ++i){
        ...
        buffer.push(distancedistance(buffer));
        res.push(buffer.slice())
    }
    return res;
}
...
function cross(firstParent, secondParent){
    ...
    if (Math.random() * 100 < chanceOfMutation){
        let rand = twoRandomNumberstwoRandomNumbers(1, lengthOfChromosome);
...
async function geneticAlggeneticAlg(){
    let firstGeneration = [];
    let end = 500;
    ...
    for(let i = 0; i < numberOfGenerations; ++i){
        ...
        if (JSON.stringify(bestChromosome) !== JSON.stringify(population[0])){
            drawTheLinesdrawTheLines(bestChromosome, population[0])
